...
class ReportMetadataTests(unittest.TestCase):
    ...
    def test_auto_and_manual_templates_expose_active_status_interactions(self):
        ...
        plate_trace_start = auto_html.index("const trace = {")
        plate_trace_end = auto_html.index("const inactiveRows", plate_trace_start)
        self.assertIn("showlegend: false", auto_html[plate_trace_start:plate_trace_end])
        plate_layout_start = auto_html.index("const layout = plotLayout({", plate_trace_end)
        plate_layout_end = auto_html.index("Plotly.react", plate_layout_start)
        self.assertIn("showlegend: false", auto_html[plate_layout_start:plate_layout_end])
        ...
        for html in (auto_html, manual_html):
            self.assertIn(".dark select option", html)
