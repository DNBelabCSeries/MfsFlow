...
| `counting_opts.gene_body_max_reads` | int | 12000000 | Target mapped fragments for PE input, or reads for SE input, per read source. PE coverage uses the primary R1 record, matching zUMIs, and stable-hash sampling avoids coordinate-order bias. Use 0 to include all reads. The denominator is summary-based and can still include supplementary alignments (FLAG 0x800) that `--primary` does not filter; this affects sampling depth (curve smoothness), not bias direction. |
