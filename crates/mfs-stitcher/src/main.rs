...
#[command(
    ...
    after_help = "Common run:\n  mfs-stitcher -i input.bam -o stitched.bam -g genes.gtf -t 20\n\nThe input BAM must be coordinate-sorted and indexed. The output BAM is coordinate-sorted and indexed.\nIsoform indexes are built in memory automatically; use --isoform and --junction together only when reusing indexes generated by --dump-index or --index-only."
)]
struct Cli {
    #[command(flatten)]
    core: CoreArgs,
...
struct CoreArgs {
    /// Coordinate-sorted and indexed input BAM file. Required for stitching.
    #[arg(
        short = 'i',
        long = "input",
    ...
    /// GTF file containing gene/transcript annotation and the selected gene attribute
...
struct OutputArgs {
    /// Coordinate-sorted, indexed output BAM containing stitched molecules
    #[arg(
        short = 'o',
        long = "output",
    ...
    /// Output 10x-compatible MEX directory (unique transcript assignments; requires isoform calling)
    #[arg(long = "matrix-out", value_name = "DIR")]
    matrix_out: Option<PathBuf>,

    /// Output isoform counts TSV, including fractional ambiguous assignments (usually .tsv.gz)
    #[arg(long = "counts-tsv", value_name = "FILE")]
    counts_tsv: Option<PathBuf>,

    /// Output molecule-level audit TSV, including transcript assignment status (usually .tsv.gz)
...
struct AnnotationArgs {
    /// Reuse the precomputed exon interval index; must be paired with --junction
    #[arg(long = "isoform", value_name = "JSON.GZ")]
    isoform: Option<PathBuf>,

    /// Reuse the precomputed exon-junction index; must be paired with --isoform
    ...
    /// Build indexes only; PREFIX is the output filename prefix and no BAM is processed
    #[arg(long = "index-only", value_name = "PREFIX")]
    index_only: Option<PathBuf>,

    /// Skip transcript isoform compatibility calling; incompatible with --matrix-out
    #[arg(long = "skip-iso")]
    skip_iso: bool,
...
struct RuntimeArgs {
    /// Number of worker threads for stitching and BAM compression
    #[arg(short = 't', long = "threads", default_value_t = 4, value_name = "N")]
    threads: usize,

    /// Treat input as single-end instead of the default paired-end mode
    #[arg(long = "single-end")]
    single_end: bool,

    /// String-valued UMI auxiliary tag in the input BAM
    #[arg(
        long = "umi-tag",
    ...
    /// String-valued cell-barcode auxiliary tag in the input BAM
    #[arg(long = "cell-tag", default_value = "CB", value_name = "TAG")]
    cell_tag: String,

    /// Keep only cell barcodes listed one per line in this file
    #[arg(long = "cells", value_name = "FILE")]
    cells: Option<PathBuf>,

    /// Process only gene IDs/names listed one per line in this file
